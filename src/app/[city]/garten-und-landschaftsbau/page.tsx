...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
    ...
    const title = `Garten- und Landschaftsbau ${city.name} `;
    const description = `Garten- und Landschaftsbau in ${city.name}. ✓ Gartenplanung ✓ Neuanlage ✓ Umgestaltung. Komplettservice vom Fachbetrieb!`;
    return {
        title, description,
        openGraph: { title, description, url: `https://garten-adler.de/${city.slug}/garten-und-landschaftsbau` },
        alternates: { canonical: `https://garten-adler.de/${city.slug}/garten-und-landschaftsbau` },
    };
}

const services = [{"title":"Gartenplanung","icon":"📐","features":["3D-Planung","Beratung","Konzept"]},{"title":"Neuanlage","icon":"🌱","features":["Rasen","Beete","Wege"]},{"title":"Umgestaltung","icon":"🔄","features":["Modernisierung","Erweiterung","Pflege"]},{"title":"Außenanlagen","icon":"🏠","features":["Einfahrten","Terrassen","Carports"]}];

export default async function CityGartenundLandschaftsbauPage({ params }: { params: Promise<{ city: string }> }) {
    const { city: citySlug } = await params;
    const city = getCityBySlug(citySlug);
    if (!city) notFound();
    ...
    const faqs = selectFAQs([
        { q: `Was kostet Garten- und Landschaftsbau in ${city.name}?`, a: `Die Kosten richten sich nach Umfang und Material. Kontaktieren Sie uns für ein kostenloses Angebot in ${city.name}.` },
        { q: `Wie schnell sind Sie in ${city.name}?`, a: `Mit nur ${city.distance}km Anfahrt können wir meist innerhalb von 1-2 Wochen starten.` },
        { q: `Bieten Sie Garten- und Landschaftsbau in ganz ${city.name} an?`, a: `Ja, wir sind in ganz ${city.name} und Umgebung tätig.` },
    ...
    const jsonLd = {
        ...
        '@graph': [
            { '@type': 'Service', name: `Garten- und Landschaftsbau ${city.name}`, provider: { '@type': 'HomeAndConstructionBusiness', name: 'Adler & Sohn', url: 'https://garten-adler.de', telephone: '04131 - 394 2971' }, areaServed: { '@type': 'City', name: city.name } },
    ...
    return (
        <>
                ...
                <div className="container-custom py-1.5">
                    <nav className="flex items-center justify-between">
                        ...
                        <div className="hidden md:flex items-center gap-6">
                            ...
                            <Link href="/garten-und-landschaftsbau" className="text-white/80 hover:text-white px-4 py-2 rounded-full transition-all">Garten- und Landschaftsbau</Link>
            ...
            <main>
                    ...
                    <div className="container-custom relative z-10">
                        ...
                        <div className="max-w-4xl">
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 mb-6">
                                ...
                                <span className="text-white/90 text-sm font-medium">Garten- und Landschaftsbau in {city.name}</span>
                            </div>
                            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
                                Garten- und Landschaftsbau in <span className="text-lime-300">{city.name}</span>
                            </h1>
                            <p className="text-xl text-white/80 leading-relaxed mb-8">Ihr Traumgarten – von der Planung bis zur Fertigstellung aus einer Hand.</p>
                            <div className="flex flex-wrap gap-4 mb-10">
                                {["Komplettservice","Individuelle Planung","Faire Preise"].map((item, i) => (
                ...
                <section className="section-padding bg-white">
                    <div className="container-custom">
                        <div className="text-center max-w-3xl mx-auto mb-12">
                            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Garten- und Landschaftsbau-Leistungen in {city.name}</h2>
                        </div>
                ...
                <section className="section-padding bg-gray-50">
                    <div className="container-custom">
                        <div className="max-w-4xl mx-auto">
                            <h2 className="text-3xl font-bold text-gray-900 mb-6">Warum Adler & Sohn für Garten- und Landschaftsbau in {city.name}?</h2>
                ...
                <section className="section-padding bg-green-50">
                    <div className="container-custom">
                        <div className="max-w-3xl mx-auto">
                            ...
                            <Link href="/blog/garten-pflegeleicht-gestalten" className="group flex items-center gap-3 p-4 bg-white rounded-xl hover:shadow-md transition-all border border-gray-100">
                                <span className="text-2xl">📖</span>
                                <span className="text-gray-700 group-hover:text-green-600 font-medium transition-colors">Garten pflegeleicht gestalten</span>
                            ...
                            <Link href="/blog/garten-winterfest-machen" className="group flex items-center gap-3 p-4 bg-white rounded-xl hover:shadow-md transition-all border border-gray-100">
                                <span className="text-2xl">📖</span>
                                <span className="text-gray-700 group-hover:text-green-600 font-medium transition-colors">Garten winterfest machen</span>
                                <svg className="w-4 h-4 text-gray-400 group-hover:text-green-500 ml-auto transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                            </Link>
                            </div>
                        </div>
                ...
                <section className="section-padding bg-white">
                    <div className="container-custom">
                        <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">FAQ – Garten- und Landschaftsbau in {city.name}</h2>
                ...
                <section className="section-padding bg-gray-900 text-white">
                    <div className="container-custom">
                        <h2 className="text-3xl font-bold mb-8 text-center">Garten- und Landschaftsbau in der Region</h2>
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {nearbyCities.map((nc) => (
                                <Link key={nc.slug} href={`/${nc.slug}/garten-und-landschaftsbau`} className="group bg-white/5 rounded-2xl p-6 border border-white/10 hover:bg-white/10 transition-all">
                ...
                <section className="section-padding bg-gradient-to-r from-green-800 to-green-700">
                    <div className="container-custom text-center">
                        <h2 className="text-3xl font-bold text-white mb-6">Garten- und Landschaftsbau in {city.name} anfragen</h2>
